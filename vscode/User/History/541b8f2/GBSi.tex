...
\end{equation}
% ALEX: Il ne faudrait pas dire que $(a, s) \in \pi$ ?
% Hed : C'est à ça que tu penses ? (cf. phrase suivante)
...
% Hed : J'ai tout commenté parce que alex a raison, on ne comprend pas à ce moment de la lecture pourquoi on parle de macro modèle. 
%     Le but est de faire le point avec la section 3 de ce chapitre, mais dans ce cas ce n'est pas ici qu'il faut faire la connexion.
 

% In MBRL, the model learns the transition function to which the reinforcement learning algorithm is subjected.
% But in other cases, the model could learn a macro transition model, $T'(s_{t+1} \mid s_t, a_{t, \dots, t+n}$, 
% giving the probability of reaching a state $s_{t+1}$ from state $s_t$ after a sequence of actions a_{t, \dots, t+n}.
% To understand the interest in such transitions, we should understand what a control-policy is.
...
% In this case, it could be interesting to simplify the problem by letting the policy, called control-policy, solving tasks that require a lower amount of actions. 
...
% % ALEX : Pas très clair pourquoi on devrait faire ça et d'où sort le macro-model. 
% % Ici, tu veux dire que tu as appris une macro pour une tâche simple et que tu veux les enchainer, quitte à
% % les séparer par des séquences arbitraires d'actions. Poiur retrouver ces 'points d'accroche', les états dans les macro 
% % deviennent tes buts. 
% % Ok, mais il faut l'écrire mieux, sinon le lecteur est perdu.
...
% Hed : La version reformulée c'est celle commenté ou celle ci-dessous ? Dans le second cas on peut supprimer tous les commentaires je pense.
