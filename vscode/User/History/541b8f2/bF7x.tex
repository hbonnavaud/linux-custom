...
But in other cases, the model could learn a macro transition model, $T'(s_{t+1} \mid s_t, a_{t, \dots, t+n}$, giving the probability of reaching a state $s_{t+1}$ from state $s_t$ after a sequence of actions a_{t, \dots, t+n} ($s$, $a_{t, \dots, t+n}$, $s'$, where $a_{t, \dots, t+n}$
is a sequence of $n$ actions). % Alex : tu n'as pas une fonction de transition à écrire ? E.g. $s' = T(s, a_{t, \dots, t+n})$
To understand the interest in such transitions,
