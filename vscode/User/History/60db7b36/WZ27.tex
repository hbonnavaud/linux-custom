\documentclass[a4paper,12pt]{article}
\usepackage[top=2cm, bottom=2cm, left=2.5cm, right=2.5cm]{geometry}
\usepackage{graphicx}
\usepackage{titlesec}
\usepackage{setspace}
\usepackage{lmodern}
\usepackage{fontspec}
\usepackage{xcolor}
\usepackage{lipsum}  % For sample text

% Define custom color
\definecolor{customColor}{RGB}{13,81,129}

% Set the main font
\setmainfont{Arial}

% Set the text color to the custom color
\color{customColor}

\begin{document}


\begin{figure}[!htb]
    \begin{minipage}{0.2\textwidth}
      \centering
      \includegraphics[width=\linewidth]{figures/logos/Republique-francaise-logo.svg.png}
    \end{minipage}\hfill
    \begin{minipage}{0.15\textwidth}
      \centering
      \includegraphics[width=\linewidth]{figures/logos/Logo_ISEA-Supaéro.png}
    \end{minipage}
    \begin{minipage}{0.22\textwidth}
      \centering
      \includegraphics[width=\linewidth]{figures/logos/Logo_universite_toulouse_2023.svg.png}
    \end{minipage}
 \end{figure}


\begin{flushright}
    \textbf{\huge Doctorat de\\ l'Université de Toulouse} \\[0.5cm]
    \textbf{\large délivré par l'ISAE-SUPAERO} \\[2cm]
    
    \noindent\rule{8cm}{0.4pt}  % Horizontal line before the title
    \\[0.5cm]
    \textbf{\large Méthodes d'exploration pour l'apprentissage par renforcement appliqué au test de systèmes critiques} \\[1.5cm]
    \noindent\rule{8cm}{0.4pt}  % Horizontal line after the title
    \\[1.5cm]
    
    \textbf{\Large Thèse présentée et soutenue, le x septembre 2047 par} \\[0.5cm]
    
    {\huge \textbf{Hedwin BONNAVAUD}} \\[2cm]
\end{flushright}

\begin{flushleft}
    \textbf{\large \textbf{École doctorale}} \\
    {\Large EDMITT - École Doctorale Mathématiques, Informatique et Télécommunications de Toulouse} \\[0.5cm]
    
    \textbf{\large \textbf{Spécialité}} \\
    {\Large Informatique et Télécommunications} \\[0.5cm]
    
    \textbf{\large \textbf{Unité de recherche}} \\
    {\Large ISAE-ONERA MOIS - Modélisation et Ingénierie des Systèmes} \\[2cm]
    
    \textbf{\large Thèse dirigée par} \\
    {\Large Emmanuel RACHELSON et Alexandre ALBORE} \\[2cm]
    
    \textbf{\large Composition du jury} \\
    {\begin{tabbing}
    % M. MICHELLE \hspace{2cm} \= Rapporteur, Institut de recherche de la saucisse de Toulouse \\
    % M. MARC \> Rapporteur, INRIA CHAPAREILLAN \\
    \end{tabbing}}
    
    \textbf{\large Membres invités} \\
    {\Large Ma maman et mon papa} 
\end{flushleft}

\end{document}
