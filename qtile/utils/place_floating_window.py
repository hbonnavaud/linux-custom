...
def place_floating_window(qtile, h_position, v_position):
    # Verify inputs
    ...
    if window:
        ...
        # Compute the available space using margin settings and screen size
        ...
        workarea_width = screen.width - left_size - right_size - settings.windows_margin * 2
        workarea_height = screen.height - top_size - bottom_size - settings.windows_margin * 2
        logger.warning("\ncomputed workarea: ")
        ...
        if h_position == "right":
            position[0] = int(0.5 * workarea_width + settings.windows_margin / 2)
        ...
        if v_position == "bottom":
            position[1] = int(0.5 * workarea_height + settings.windows_margin / 2)
        ...
        if h_position == "left" or h_position == "right":
            dimention[0] = int(0.5 * workarea_width - settings.windows_margin / 2)
        ...
        if v_position == "top" or v_position == "bottom":
            dimention[1] = int(0.5 * workarea_height - settings.windows_margin / 2)
            ...
            logger.warning(" > Setting dimention[1] to " + str(dimention[1]))
        borders_width = 2 * settings.windows_border_width
